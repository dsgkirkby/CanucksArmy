from constants import league_start_years
...
def get_player_stats(league, season, results_array, goalie_results_array, show_multiple_teams=False, playoffs=False):
    ...
    standings_url = "http://www.hockeydb.com/ihdb/stats/leagues/seasons/{0}{1}{2}.html".format(
        league,
        league_start_years.get_league_start_year(league),
        season,
    )

    standings_page = BeautifulSoup(requests.get(standings_url).text, "html.parser")
    ...
    def team_table(tag):
        if (tag.has_attr('id')):
            print(tag.attr('id'))
        return tag.has_attr('id') and tag.attr('id') == 'sortableTable0'

    def team_row(tag):
        return tag.name == 'tr' and (not tag.has_attr('class') or 'hid' not in tag.attr('class'))

    teams = standings_page.find(team_table)#.find_all(team_row)
